    ...
    pub fn new(title: &str, width: u32, height: u32, app: App) -> Result<Box<Self>> {
        unsafe {
            ...
            let path_tb_rect = Rect {
                X: Self::PADDING, 
                Y: Self::PADDING, 
                Width: width as f32 - Self::BTN_WIDTH * 2.0 - Self::PADDING * 4.0, 
                Height: Self::ONELINE_HEIGHT
            };
            result.controls.insert(Self::ID_TEXTBOX_PATH, path_tb_rect);
            CreateWindowExW(
                WINDOW_EX_STYLE::default(),
                w!("EDIT"),
                w!(""),
            ...
            let result_tb_rect = Rect {
                ...
                Width: width as f32 - Self::PADDING * 2.0, 
                Height: height as f32 - 
                    Self::ONELINE_HEIGHT - 
                    path_tb_rect.Y - 
                    Self::PADDING * 2.0
            };
            result.controls.insert(Self::ID_TEXTBOX_RESULT, result_tb_rect);
            CreateWindowExW(
                WINDOW_EX_STYLE::default(),
                w!("EDIT"),
                w!(""),
            ...
            result.controls.insert(Self::ID_BTN_PATH, path_btn_rect);
            CreateWindowExW(
                WINDOW_EX_STYLE::default(),
                w!("BUTTON"),
                hstr_to_pcwstr(&result.local.path),
                WS_VISIBLE | WS_CHILD,
            ...
            result.controls.insert(Self::ID_BTN_REMOVE, remove_btn_rect);
            CreateWindowExW(
                WINDOW_EX_STYLE::default(),
                w!("BUTTON"),
                hstr_to_pcwstr(&result.local.remove),
                WS_VISIBLE | WS_CHILD,
        ...
        unsafe {
            self.controls.iter().for_each(|(id, rect)| {
                if let Ok(hwnd) = GetDlgItem(self.app_wnd, *id as i32) {
                    let _ = Self::set_window_pos(hwnd, rect);
                }
            });
        }
    }

    fn set_window_pos(hwnd: HWND, rect: &Rect) -> Result<()> {
        unsafe {
            SetWindowPos(
                hwnd,
                None,
